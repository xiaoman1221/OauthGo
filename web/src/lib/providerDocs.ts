...
export type ProviderSchemaRaw = Partial<ProviderSchema> & {
  name?: string
  displayName?: string
  category?: string
}
...
export const channelList: ProviderSchemaRaw[] = Object.values(channelSchemas)

export const defaultChannelSchema: ProviderSchema = {
  idLabel: 'ClientID',
...
export function channelSchema(name: string): ProviderSchema {
  return { ...defaultChannelSchema, ...(channelSchemas[name] || {}) } as ProviderSchema
}

// 便捷别名（兼容页面引用）
export const channelListTyped: ProviderSchema[] = channelList as ProviderSchema[]
export const channelSchemaTyped = (name: string): ProviderSchema => channelSchema(name)
